...
def compf(x,y):
    if len(x.split("."))>len(y.split(".")):
        maximum = len(y.split(".")) -1
    else:
        maximum = len(x.split(".")) -1
...
def compblacklist(addon):
    print("checking if addon is blacklisted: ".encode('utf-8') + addon.encode('utf-8'))
    BLAF=getblacklist(BlackListAddonFIle)
...
def extract_art_from_zip(base):
    ...
    for addon in addons:
            try:
                ...
                print("cant scan ".encode('utf-8'),e," - ",addon.encode('utf-8') )
            ...
            if key in old_dict.keys():
                try:
                    if compf(new_dict[key],old_dict[key]) == True:
                        ...
                        print("no need to update existing key " + key )
                except Exception as e:
